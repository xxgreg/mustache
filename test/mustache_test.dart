...
//		});
  ...
  group('Lambdas', () {
    void _lambdaTest({required template, lambda, output}) => expect(
  ...
  group('Mirrors', () {
    test('Simple field', () {
      var output = parse('_{{bar}}_').renderString(Foo()..bar = 'bob');
      expect(output, equals('_bob_'));
    });

    test('Simple field', () {
      var output = parse('_{{jim}}_').renderString(Foo());
      expect(output, equals('_bob_'));
    });

    test('Lambda', () {
      var output =
          parse('_{{lambda}}_').renderString(Foo()..lambda = (_) => 'yo');
...
Object? renderFail(source, values) {
  try {
